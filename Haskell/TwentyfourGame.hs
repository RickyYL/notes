...
buildExp (es1, es2) = [op e1 e2 | e1 <- es1, e2 <- es2, op <- [Plus,Sub,Mult,Div]]

toExp :: [Double] -> [Exp]
toExp []  = []
toExp [x] = [Val x]
toExp xs  = concat [buildExp (toExp l, toExp r) | (l,r) <- divide xs]

generate :: [Double] -> [Exp]
generate ns = concatMap toExp (permutations ns)
